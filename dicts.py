...
def count_words(phrase):
    ...
    word_counted = {}
        for work in words: 
            word counted[word]= word_counted.get(word,0)
            +1
        return word_counted
...
def print_melon_at_price(price):
    ...
    If there are no melons at that price print "None found"
        ...
        None found
    """
melon_dict= {2.50: ["Honeydew", "Cantaloupe"], 2.95: ["Watermelon"], 3.25: ["Musk", "Crenshaw"], 14.25: ["Christmas"]}
    

      if item in melon_dict:
        for item melon_dict: (print_melon_at_price,"none")
        return 
...
def translate_to_pirate_talk(phrase):
    ...
    for word in words:
        if word in pirate_dict:
            new_translation.append(pirate_dict[word])
        else:
            new_translation.append(word)
